...
function createReadOnlyTools(executor: ToolExecutor) {
    return {
        ...
        search_files: tool({
            description: "Find files matching a glob pattern; optinal content filter.",
            inputSchema: z.object({
                root: z.string(),
                pattern: z.string(),
...
export async function runAgent(ctx: {
    ...
    await finishOrApprove(
        ctx, chatId, tracker, executor, '✅ Done. No file changes were needed.'
    );
}
