    ...
    return {
        ...
        search_files: tool({
            ...
            inputSchema: z.object({
                ...
                pattern: z.string().describe("Glob-like pattern using * and ** (forward slashse)"),
        ...
        analyze_codebase: tool({
            description: "Summarize structure: file counts , size , extensions , Read-olny.",
            inputSchema: z.object({
        ...
        read_skill: tool({
            ...
            execute: async ({ path: p }) =>
                executor.readSkill(p),
        }),
    };
}

const PLAN_INSTRUCTIONS = (
    codebase: string, hasWeb: boolean
) =>
    [
        ...
        hasWeb
            ? 'Web tools are available (web_search/web-crawl/fetch_url). Use only when needed.'
...
export async function generatePlan(goal: string) {
    ...
    return {
        goal, researchSummary: validated.researchSummary, steps
    }
}
